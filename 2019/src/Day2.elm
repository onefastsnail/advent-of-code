...
evaluatePermutation : ( Int, Int ) -> Int -> Computer -> Bool
evaluatePermutation permutation desiredOutput computer =
    let
        output =
...
runPermutations state desiredOutput permutations =
    List.foldl
        (\a b ->
            let
                result =
                    runPermutation a state |> evaluatePermutation a desiredOutput
            in
            case result of
                True ->
                    a :: b
