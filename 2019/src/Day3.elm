...
parsePuzzleInput : String -> List (List String)
parsePuzzleInput str =
    String.split "\n" str
        |> List.map (\a -> String.split "," a)
...
nextPoint : String -> ( Int, Int ) -> ( Int, Int )
nextPoint direction ( x, y ) =
    let
        value =
            String.slice 1 5 direction
                |> String.toInt
                |> Maybe.withDefault 0
    in
    case String.slice 0 1 direction of
        "U" ->
            ( x, y + value )
...
fillPoints : ( Int, Int ) -> ( Int, Int ) -> List ( Int, Int )
fillPoints ( x0, y0 ) ( x1, y1 ) =
    let
        xDistance =
            getLowestNumber x0 x1

        yDistance =
            getLowestNumber y0 y1
...
convertDirectionsToPoints : List String -> List ( Int, Int )
convertDirectionsToPoints directions =
    List.foldl
        (\a b ->
            let
                z =
...
findIntersections : List (List ( Int, Int )) -> List ( Int, Int )
findIntersections paths =
    let
        ...
        intersections =
            Set.intersect (Set.fromList path1) (Set.fromList path2)
    in
    intersections |> Set.toList


getAnswerPart1 : String -> Int
getAnswerPart1 puzzle =
    parsePuzzleInput puzzle
        |> List.map convertDirectionsToPoints
        |> findIntersections
        |> List.map (\a -> calculateManhattanDistance ( 0, 0 ) a)
        |> List.sort
        |> List.filter (\a -> a > 0)
        |> List.minimum
        |> Maybe.withDefault 0
